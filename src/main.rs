...
trait Instruction {
    fn execute(&self, cpu: &mut CPU, current_tick: u8);
}
...
impl Instruction for LDA {
    fn execute(&self, cpu: &mut CPU, current_tick: u8) {
        ...
        if current_tick < ticks {
            // In the middle of execution, just return
            return;
        }
...
mod tests {
    ...
    fn test_lda_immediate_positive_value() {
        ...
        assert_eq!(cpu.accumulator, 0x42, "Accumulator should contain loaded value");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
    ...
    fn test_lda_immediate_zero_value() {
        ...
        assert!(cpu.is_flag_set(StatusFlags::Zero), "Zero flag should be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
    }
    ...
    fn test_lda_immediate_negative_value() {
        ...
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(cpu.is_flag_set(StatusFlags::Negative), "Negative flag should be set");
    }
    ...
    fn test_lda_zero_page() {
        ...
        assert_eq!(cpu.accumulator, 0x33, "Accumulator should contain value from zero page");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
        assert_eq!(cpu.program_counter, 0x8001, "PC should increment by 1");
    }
    ...
    fn test_lda_zero_page_x() {
        ...
        assert_eq!(cpu.accumulator, 0x77, "Accumulator should contain value from zero page,X");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
    }
    ...
    fn test_lda_zero_page_x_wraparound() {
        ...
        assert_eq!(cpu.accumulator, 0x99, "Accumulator should contain value from wrapped zero page address");
    }
    ...
    fn test_lda_absolute() {
        ...
        assert_eq!(cpu.accumulator, 0xAB, "Accumulator should contain value from absolute address");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(cpu.is_flag_set(StatusFlags::Negative), "Negative flag should be set for 0xAB");
    ...
    fn test_lda_absolute_x_no_page_boundary() {
        ...
        assert_eq!(cpu.accumulator, 0x55, "Accumulator should contain value from absolute,X");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
    }
    ...
    fn test_lda_absolute_x_page_boundary_crossing() {
        ...
        assert_eq!(cpu.accumulator, 0x88, "Accumulator should contain value from absolute,X with page crossing");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(cpu.is_flag_set(StatusFlags::Negative), "Negative flag should be set for 0x88");
    }
    ...
    fn test_lda_absolute_y() {
        ...
        assert_eq!(cpu.accumulator, 0x66, "Accumulator should contain value from absolute,Y");
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set");
    }
    ...
    fn test_lda_flags_cleared_before_setting() {
        ...
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should be cleared");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should be cleared");
    }
    ...
    fn test_lda_execution_timing() {
        ...
        assert_eq!(cpu.accumulator, 0xFF, "Accumulator should not change with insufficient ticks");
        assert_eq!(cpu.program_counter, 0x8000, "PC should not change with insufficient ticks");
        
        // Execute with sufficient ticks - should complete
        lda.execute(&mut cpu, 2);
        
        assert_eq!(cpu.accumulator, 0x42, "Accumulator should change with sufficient ticks");
        assert_eq!(cpu.program_counter, 0x8001, "PC should advance with sufficient ticks");
    }
    ...
    fn test_lda_zero_page_timing() {
        ...
        assert_eq!(cpu.accumulator, 0x00, "Should not execute with insufficient ticks");
        
        // Test sufficient ticks
        lda.execute(&mut cpu, 3);
        assert_eq!(cpu.accumulator, 0x33, "Should execute with sufficient ticks");
    }
    ...
    fn test_lda_absolute_timing() {
        ...
        // Test sufficient ticks
        lda.execute(&mut cpu, 4);
        assert_eq!(cpu.accumulator, 0xAB, "Should execute with sufficient ticks");
    ...
    fn test_lda_boundary_values() {
        ...
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(!cpu.is_flag_set(StatusFlags::Negative), "Negative flag should not be set for 0x7F");
    }
    ...
    fn test_lda_boundary_values_negative() {
        ...
        assert!(!cpu.is_flag_set(StatusFlags::Zero), "Zero flag should not be set");
        assert!(cpu.is_flag_set(StatusFlags::Negative), "Negative flag should be set for 0xFF");
    }
    ...
    fn test_lda_memory_addressing_edge_cases() {
        ...
        assert_eq!(cpu.accumulator, 0x55, "Should load from zero page address 0x00");
    }
    ...
    fn test_lda_memory_addressing_edge_cases_high() {
        ...
        assert_eq!(cpu.accumulator, 0xAA, "Should load from zero page address 0xFF");
    }
    ...
    fn test_lda_program_counter_advancement() {
        ...
        assert_eq!(cpu.program_counter, 0x8001, "PC should advance by 1 for immediate mode");
        ...
        assert_eq!(cpu.program_counter, 0x8001, "PC should advance by 1 for zero page mode");
    }
    ...
    fn test_lda_absolute_pc_advancement() {
        ...
        assert_eq!(cpu.program_counter, 0x8002, "PC should advance by 2 for absolute mode");
    }
}
